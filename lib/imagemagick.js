...
var process = require('process'),
  path = require('path'),
  cwd = process.env.IMAGEMAGICKPATH || '';
...
ImageMagick = (function() {
  ...
  ImageMagick.prototype.identify = function(filepath, callback) {
    ...
    return this.exec( path.join(cwd, 'identify') + ' ' + filepath, function(error, stdout, stderr) {
  ...
  ImageMagick.prototype.composite = function(options, callback) {
    ...
    command = path.join(cwd, 'convert') + " -size " + width + "x" + height + "      canvas:transparent      -alpha transparent      " + filepath + "    ";
  ...
  ImageMagick.prototype.composeImage = function(filepath, image, downsampling, callback) {
    var command;
    command = path.join(cwd, 'composite')  + "   -geometry " + image.width + "x" + image.height + "+" + image.cssx + "+" + image.cssy + "      ";
    if (downsampling) {
